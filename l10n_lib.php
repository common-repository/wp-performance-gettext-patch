<?php
/**
 * implemented nativ Gettext - Lib
 * 
 * Code - http://core.trac.wordpress.org/ticket/17268 (Linushoppe)
 * 
 * Changes:
 * 	- add Cache-control
 *  - other minor changes
 * 
 * Modifications by Julius Fischer
 * http://www.it-gecko.de
 * 
 * Version 0.11
 */


define('WP_P_GT_P_CACHE_STR', '+'.get_option('wp_p_gt_cachetime', time()));
define('WP_P_GT_P_CACHE_DIR', realpath(ABSPATH.'/wp-content/plugins/wp-performance-gettext-patch/wp-lang/').'/');
...
{
	$trans = dgettext($domain.WP_P_GT_P_CACHE_STR, $text);

	if($domain == 'default')
	{
		if($text == $trans)
			$trans = dgettext('default_admin'.WP_P_GT_P_CACHE_STR, $text);
		
		if($text == $trans)
			$trans = dgettext('default_network_admin'.WP_P_GT_P_CACHE_STR, $text);
		
		if($text == $trans)
			$trans = dgettext('default_ms'.WP_P_GT_P_CACHE_STR, $text);
	}
	
	return apply_filters('gettext', $trans, $text, $domain);
}
...
{
	$translation = dgettext($domain.WP_P_GT_P_CACHE_STR, $context."\x04".$text);
	
	if($domain = 'default')
	{
		if($translation == $context."\x04".$text)
			$translation = dgettext('default_admin'.WP_P_GT_P_CACHE_STR, $context."\x04".$text);
		
		if($translation == $context."\x04".$text)
			$translation = dgettext('default_network_admin'.WP_P_GT_P_CACHE_STR, $context."\x04".$text);
		
		if($translation == $context."\x04".$text)
			$translation = dgettext('default_ms'.WP_P_GT_P_CACHE_STR, $context."\x04".$text);
	}
...
{
	$translation = dngettext($domain.WP_P_GT_P_CACHE_STR, $single, $plural, $number); 

	if($domain == 'default')
	{
		if($translation == $plural)
			$translation = dngettext('default_admin'.WP_P_GT_P_CACHE_STR, $single, $plural, $number);
			
		if($translation == $plural)
			$translation = dngettext('default_network_admin'.WP_P_GT_P_CACHE_STR, $single, $plural, $number); 
		
		if($translation == $plural)
			$translation = dngettext('default_ms'.WP_P_GT_P_CACHE_STR, $single, $plural, $number); 
	}
...
{
	$translation = dngettext($domain.WP_P_GT_P_CACHE_STR, $context."\x04".$single, $plural, $number); 
...
function load_textdomain( $domain, $mofile )
{
	...
	if (!is_file(WP_P_GT_P_CACHE_DIR.$locale.'/LC_MESSAGES/'.$domain.WP_P_GT_P_CACHE_STR.'.mo') && is_readable($mofile) && (is_dir(WP_P_GT_P_CACHE_DIR.$locale.'/LC_MESSAGES') 
																								|| mkdir (WP_P_GT_P_CACHE_DIR.$locale.'/LC_MESSAGES', 0775, true))) 
 			copy($mofile, WP_P_GT_P_CACHE_DIR.$locale.'/LC_MESSAGES/'.$domain.WP_P_GT_P_CACHE_STR.'.mo'); 
 		                 
	bindtextdomain($domain.WP_P_GT_P_CACHE_STR, WP_P_GT_P_CACHE_DIR); 
	bind_textdomain_codeset($domain.WP_P_GT_P_CACHE_STR, 'UTF-8'); 
...
function load_default_textdomain() {
	...
	if ( ( is_multisite() || ( defined( 'WP_INSTALLING_NETWORK' ) && WP_INSTALLING_NETWORK ) ) && ! file_exists(  WP_LANG_DIR . "/admin-$locale.mo" ) ) {
		load_textdomain( 'default_ms', WP_LANG_DIR . "/ms-$locale.mo" );
		return;
	}

	if ( is_admin() || ( defined( 'WP_REPAIRING' ) && WP_REPAIRING ) )
		load_textdomain( 'default_admin', WP_LANG_DIR . "/admin-$locale.mo" );

	if ( is_network_admin() || ( defined( 'WP_INSTALLING_NETWORK' ) && WP_INSTALLING_NETWORK ) )
		load_textdomain( 'default_network_admin', WP_LANG_DIR . "/admin-network-$locale.mo" );
